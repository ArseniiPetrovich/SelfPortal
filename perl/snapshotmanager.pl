...
sub get_child_snapshot {
	my $shot_tree = shift;
	my @shots=();
	my $vm=Vim::get_view(mo_ref=>$shot_tree->vm);
	my %shot = (
			'id' => $shot_tree->snapshot->value,
			'vmid' => $vm->summary->config->uuid,
			'name' => $vm->name,
			'createddate' => $shot_tree->createTime, 
	);
	push (@shots,\%shot);
	my @childshotlist=$shot_tree->childSnapshotList;
	if (defined ($childshotlist[0][0]))	
	{
		my @childshotlist=$shot_tree->childSnapshotList;		
		push (@shots,get_child_snapshot($childshotlist[0][0]));
	}
	return @shots;
}
...
sub list_snapshot {
   ...
   foreach (@$vm_views) {
      ...
      if(defined $snapshots) {
	  	my @rootshotlist=$_->snapshot->rootSnapshotList;
		push (@shots,get_child_snapshot($rootshotlist[0][0]));		
		
      }
   }
   print encode_json \@shots;
}
...
sub create_snapshot {
   ...
   foreach (@$vm_views) {
      ...
      my $hostname = Vim::get_view(mo_ref => $mor_host)->name;
      eval {
      my $snapshot=$_->CreateSnapshot(name => $snapshot_name,
...
sub find_snapshot_name {
   ...
   my $count = 0;
   foreach my $node (@$tree) {
      if ($node->name eq $name || $node->snapshot->value eq $name) {
         $ref = $node;
         $count++;
      }
